...
@section('subhead')
    <title>empresa - Editar</title>
@endsection

@section('subcontent')
    <div class="intro-y mt-8 flex items-center">
        <h2 class="mr-auto text-lg font-medium">Editar empresa: {{ $empresa->nombre }}</h2>
    </div>
        ...
        <div class="intro-y col-span-12 lg:col-span-12">
            <div class="intro-y box p-5">
                <form method="POST" action="{{ route('empresa.update', ['id' => $empresa->id]) }}" enctype="multipart/form-data">
                    ...
                    <div class="intro-y col-span-12 lg:col-span-6">
                        <x-base.form-label for="nombre">Etiqueta del empleado</x-base.form-label>
                        <x-base.form-input
                            ...
                            value="{{ old('etiqueta_empleado', $empresa->etiqueta_empleado) }}"
                        />
                    ...
                    <div class="intro-y col-span-12 lg:col-span-6 mt-4">
                        ...
                        >
                            <option value="1" {{ old('status', $empresa->status) == 1 ? 'selected' : '' }}>Activo</option>
                            <option value="0" {{ old('status', $empresa->status) == 0 ? 'selected' : '' }}>Inactivo</option>
                        </select>
                        @error('status')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    @if($empresa->logo)
                        <div class="intro-y col-span-12 mt-4">
                            ...
                            <div>
                                <img src="{{ asset('storage/' . $empresa->logo) }}" alt="Logo Actual" class="w-32 h-32 object-cover mt-2">
                            </div>
                        </div>
                    @endif
